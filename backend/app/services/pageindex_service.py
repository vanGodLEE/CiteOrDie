...
class PageIndexService:
    ...
    def __init__(
        ...
        Args:
            model: LLM模型名称（None则使用配置中的STRUCTURIZER_MODEL）
        ...
        """
        # 如果未指定模型，使用配置中的STRUCTURIZER_MODEL
        self.model = model if model is not None else settings.structurizer_model
        ...
        logger.info(f"  - 429限流自动降级: 由API层处理（utils.py）")

    def parse_pdf(self, pdf_path: str) -> Dict[str, Any]:
        """
        解析PDF文件，生成文档树结构
        
        429限流自动降级由pageindex/utils.py的API层处理，此处无需重试逻辑
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"PageIndex解析失败: {error_msg}")
...
def get_pageindex_service() -> PageIndexService:
    ...
    429限流自动降级由pageindex/utils.py的API层处理
    """
