...
def mineru_parser_node(state: TenderAnalysisState) -> Dict[str, Any]:
    ...
    # 更新任务进度
    update_progress(task_id, 22, "🔬 启动MinerU深度解析...")
    log_step(task_id, "初始化MinerU PDF解析引擎")
    
    try:
        logger.info(f"开始调用MinerU解析PDF: {pdf_path}")
        log_step(task_id, "扫描PDF页面，识别文本、图片、表格")
        
        # 调用MinerU服务
        mineru_service = get_mineru_service()
        
        update_progress(task_id, 25, "📊 解析文档内容（文本+图表）...")
        log_step(task_id, "提取页面文本和布局信息")
        ...
        log_step(task_id, "MinerU解析完成，处理结果数据")
