"""
Clause Locator节点 - 为条款定位positions

遍历所有节点的clauses，为每个条款填充positions字段：
- 图片/表格条款 → 直接使用节点的positions
- 文本条款 → 使用RequirementTextMatcher智能匹配
"""

from typing import Dict, Any, List
from loguru import logger
from app.core.states import TenderAnalysisState, PageIndexNode, ClauseItem
...
def requirement_locator_node(state: TenderAnalysisState) -> Dict[str, Any]:
    ...
    输入：
    - state.pageindex_document: 包含所有节点和条款的文档树
    ...
    2. 判断条款来源（图片/表格 vs 文本）
    3. 图片/表格条款 → 直接使用节点positions
    4. 文本条款 → 使用RequirementTextMatcher智能匹配
    5. 填充positions字段
    """
    logger.info("=" * 60)
    logger.info("条款位置定位节点开始执行")
    logger.info("=" * 60)
    ...
    visual_requirements = 0  # 图片/表格条款
    text_requirements = 0    # 文本条款
    ...
    for root_node in pageindex_doc.structure:
        ...
        for node in all_nodes:
            ...
            logger.info(f"处理节点 '{node.title}' 的 {len(node.clauses)} 个条款")
            ...
            for req in node.clauses:
                ...
                if positions:
                    ...
                    logger.warning(
                        f"  ❌ 条款定位失败: {req.matrix_id}\n"
                        f"     条款概述: {req.original_text[:50]}...\n"
                        f"     原文片段: {req.original_text[:50]}..."
                    )
    
    # 输出统计
    logger.info(f"✓ 条款位置定位完成")
    logger.info(f"  - 总条款数: {total_requirements}")
    logger.info(f"  - 图片/表格条款: {visual_requirements}")
    logger.info(f"  - 文本条款: {text_requirements}")
...
def _locate_visual_requirement(
    requirement: ClauseItem,
    node: PageIndexNode,
    mineru_content_list: List[Dict[str, Any]]
) -> List[List[int]]:
    """
    定位图片/表格条款的positions（✅ 优化：基于img_path精确匹配）
    ...
    Args:
        requirement: 条款对象（可能包含img_path字段）
        node: 节点对象
    ...
    if requirement.img_path:
        ...
        if positions:
            logger.debug(
                f"✓ 图片/表格条款 {requirement.matrix_id}: "
                f"通过img_path精确匹配 ({requirement.img_path})"
            )
            return positions
        else:
            logger.warning(
                f"⚠️  条款 {requirement.matrix_id} 的img_path={requirement.img_path} "
...
def _locate_text_requirement(
    ...
    if not node_content_list:
        logger.warning(
            ...
            f"无法为条款 {requirement.matrix_id} 定位positions"
        )
        return []
