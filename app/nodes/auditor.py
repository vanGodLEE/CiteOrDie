...
from app.core.states import TenderAnalysisState, ClauseItem
...
def auditor_node(state: TenderAnalysisState) -> dict:
    ...
    if not clauses:
        logger.warning("没有提取到任何条款")
        return {"final_matrix": []}
    
    logger.info(f"开始处理 {len(clauses)} 条原始条款（不去重）")
    
    # 步骤1: 按章节排序（不去重）
    sorted_reqs = _sort_requirements(clauses)
    
    # 步骤2: 格式统一
    final_matrix = _normalize_requirements(sorted_reqs)
    
    logger.info(f"====== Auditor完成，最终输出 {len(final_matrix)} 条条款 ======")
    _print_summary(final_matrix)
    
    return {"final_matrix": final_matrix}


def _sort_requirements(requirements: List[ClauseItem]) -> List[ClauseItem]:
    """
    排序条款：按章节编号和matrix_id排序
    """
    sorted_reqs = sorted(
        requirements,
...
def _normalize_requirements(requirements: List[ClauseItem]) -> List[ClauseItem]:
    """
    格式统一：去除多余空格
    """
...
def _print_summary(requirements: List[ClauseItem]):
    """打印条款摘要统计"""
    # 按章节统计
    section_stats = {}
    for req in requirements:
